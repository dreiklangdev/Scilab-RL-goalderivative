...
class NormalizeDictObservation(
    gym.ObservationWrapper[WrapperObsType, ActType, ObsType],
    gym.utils.RecordConstructorArgs,
):
    ...
    def __init__(self, env: gym.Env[ObsType, ActType], epsilon: float = 1e-8):
        ...
        assert env.observation_space['observation'].shape is not None

        self.obs_rms = RunningMeanStd(
            shape=env.observation_space['observation'].shape, dtype=self.observation_space.dtype
        )
    ...
    def observation(self, observation: ObsType) -> WrapperObsType:
        ...
        if self._update_running_mean:
            self.obs_rms.update(observation['observation'])
        return dict(
            observation=np.float32(
                (observation['observation'] - self.obs_rms.mean) / np.sqrt(self.obs_rms.var + self.epsilon)
            ),
